...
public class Footle
{
	...
	private static void writeLLVM(CodeAndReg compiledCode, Env env, ArrayList<LLVMLine> funcdecs, ArrayList<Integer> funcids)
	{
		...
		try{
		 ...
		 try{
			...
			output.write(env.getMallocReg() + " = malloc {%eframe*, i32, [" + env.numIds() + " x i32]}\n");
			output.write(env.getCurrentScope() + " = bitcast {%eframe*, i32, [" + env.numIds() + " x i32]}* " + env.getMallocReg() + " to %eframe*\n");
